...
// --- Section component stays unchanged! ---
const Section = ({ title, children, openByDefault = false }) => {
  const [open, setOpen] = useState(openByDefault);
  return (
    <View style={styles.sectionCard}>
...
export default function EnterpriseForm({ navigation, route }) {
  ...
  // const menuItems = [
  //   { label: t(language, 'general'), onPress: () => navigation.popToTop() },
  //   { label: t(language, 'docs_photos'), onPress: () => navigation.popToTop() },
  //   {
  //     label: 'Logout',
  //     color: '#EE6969',
  //     onPress: async () => {
  ...
  return (
    <ScrollView contentContainerStyle={{ padding: 12 }}>
      ...
      {/* Language Toggle just under header */}
      {/* <LanguageToggle style={{ alignSelf: 'flex-end', marginBottom: 10 }} /> */}
...
const styles = StyleSheet.create({
  ...
  inputLabel: {      // new style for input labels
    fontWeight: '600',
    marginBottom: 6,
  },
  redButton: {
  ...
  inputStyle: {      // rename your existing inputStyle and adjust marginBottom
    borderWidth: 1,
    borderColor: '#EE6969',
    padding: 10,
