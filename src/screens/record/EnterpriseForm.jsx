    ...
    <ScrollView contentContainerStyle={{ padding: 12 }}>
      ...
      <Section title="Finance">
        {['initial_investment','source_of_investment','working_capital_monthly','annual_turnover','profit_percentage','loan_details'].map(key => (
          <View key={key}>
            <Text>{key.replace(/_/g,' ').replace(/\b\w/g, l => l.toUpperCase())}</Text>
            <TextInput value={form[key]} onChangeText={v => setField(key, v)} style={inputStyle} multiline={['loan_details'].includes(key)} keyboardType={['initial_investment','working_capital_monthly','annual_turnover','profit_percentage'].includes(key) ? 'numeric' : 'default'} />
          </View>
        ))}
      </Section>
      ...
      <Section title="Marketing & Sales">
        {['target_customers','marketing_channels','monthly_sales','marketing_strategy','marketing_challenges'].map(key => (
          <View key={key}>
            <Text>{key.replace(/_/g,' ').replace(/\b\w/g, l => l.toUpperCase())}</Text>
            <TextInput value={form[key]} onChangeText={v => setField(key, v)} style={inputStyle} multiline={['marketing_strategy','marketing_challenges'].includes(key)} keyboardType={key==='monthly_sales'?'numeric':'default'} />
          </View>
        ))}
      </Section>
