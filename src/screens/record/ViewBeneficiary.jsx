...
const BENEFICIARY_FIELDS = [
  'name','dob','gender','phone','other_contact','shg_joining_date','marital_status','education',
  'parent_spouce_name','relation','religion','social_category','remarks',
];

const ENTERPRISE_FIELDS = [
  'enterprise_name','enterprise_type','ownership_type','year_of_establishment','raw_material',
  'machinery_equipment','workplace_type','electricity_available','water_available',
  ...
  'market_linkage','mentorship_support','expansion_plan','required_support','photo_enterprise',
  'photo_entrepreneur','photo_product','certificate_docs','recorded_by_user_id','created_at','updated_at','deleted_at',
];
        ...
        {loading ? (
          ...
          <>
            {ENTERPRISE_FIELDS.map(key => {
              ...
              // photos / certificate docs clickable
            ...
            <TouchableOpacity style={styles.button} onPress={() => navigation.navigate('EnterpriseForm', { beneficiary })}>
              <Text style={styles.buttonText}>Edit Enterprise</Text>
            </TouchableOpacity>
          </>
        ) : (
          <>
            <Text style={{ color: '#666' }}>No enterprise record found for this beneficiary.</Text>
            <TouchableOpacity style={styles.button} onPress={() => navigation.navigate('EnterpriseForm', { beneficiary })}>
              <Text style={styles.buttonText}>Record Enterprise</Text>
            </TouchableOpacity>
          </>
        )}
      </View>
...
const styles = StyleSheet.create({
  container: { padding: 12 },
  headerRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10, marginTop: 50 },
  heading: { fontSize: 18, fontWeight: '700', marginBottom: 10 },
  ...
  col: { marginBottom: 8 },
  label: { fontWeight: '600', color: '#333', width: '45%' },
  value: { color: '#222', flex: 1, flexShrink: 1 },  // ✅ allow wrapping
  link: { color: '#EE6969', flex: 1, flexShrink: 1 }, // ✅ allow wrapping for links
