...
export default function LoginScreen({ navigation }) {
  ...
  const [selectedRole, setSelectedRole] = useState('CRP'); // UI-only selection
  ...
  const doLogin = async () => {
    ...
    setLoading(true);
    try {
      // NOTE: only username & password are sent to GAS (role is UI-only)
      const res = await gsApi.login(username, password);

      if (res && res.success) {
        await saveUser(res.user);
        // Use server-side role to decide navigation (server is authority)
        ...
        Alert.alert('Login failed', res && (res.message || res.text) ? (res.message || res.text) : 'Invalid credentials');
      }
    } catch (err) {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.roleRow}>
        <TouchableOpacity
          style={[styles.roleButton, selectedRole === 'CRP' ? styles.roleButtonActive : null]}
          onPress={() => setSelectedRole('CRP')}
        >
