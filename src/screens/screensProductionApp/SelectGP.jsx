...
export default function SelectGP({ navigation, route }) {
  ...
  const pageSize = 10; // Set to match your backend pagination size
  ...
  // Handle missing adminBlockId inside useEffect (unconditional hook call)
  useEffect(() => {
    if (!adminBlockId) {
      Alert.alert('Error', 'No block selected.');
      navigation.goBack();
    ...
    const fetchGPs = async () => {
      setLoading(true);
      try {
        // Assume API accepts (blockId, page, query) parameters
